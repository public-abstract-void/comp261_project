...
def portfolio_performance(df):
    ...
    print("Avg Long-Short:", (long_returns + short_returns).mean())
...
def decile_analysis(df):
    ...
    for _, g in df.groupby("date"):
        ...
        # ----------------------------------------------------
        # FIX: HANDLE DUPLICATE BIN EDGES
        # ----------------------------------------------------
        try:
            g["decile"] = pd.qcut(
                g["prediction"],
    ...
    if len(decile_returns) == 0:
        print("⚠️ Could not compute deciles (predictions too flat)")
        return
